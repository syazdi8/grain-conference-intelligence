...
| `js/planning.js` | Coverage flags: unowned A-tier events; A/B events in the same city within 14 days ("Trip cluster": travel that could plausibly be combined, a signal for the sales lead, not a recommendation); 3+ A/B events starting in any rolling 3-week window, wherever they are, with overlapping windows grouped into one stretch that can therefore run longer than 3 weeks ("Busy stretch": concentrated coverage demand). Which contacts we met at past editions of each event ("Relationships from past editions" on the conference page: where those relationships stand today, not an attendee list or pipeline). Also which conferences Capture offers: only those happening on the app date, so an encounter is never logged against an event that hasn't happened (logging after an event is future work) |
